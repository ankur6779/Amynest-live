...
export function exportPublishedVideo(
  video: PublishedVideo,
  format: PublishedExportFormat = "json",
): PublishedExportResult {
  switch (format) {
    case "json":
      return {
        format: "json",
        content: `${stableStringify(video)}\n`,
        contentType: "application/json",
      };
    case "yaml":
      return {
        format: "yaml",
        content: `${toYaml(video)}\n`,
        contentType: "application/yaml",
      };
    case "publish-manifest-v1":
      return {
        format: "publish-manifest-v1",
        ...
        contentType: "application/vnd.amynest.publish-manifest.v1+json",
      };
    default: {
      const _exhaustive: never = format;
      throw new Error(`Unsupported publish export format: ${_exhaustive}`);
    }
  }
}
