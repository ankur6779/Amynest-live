/**
 * Google AI Studio production run — golden-002 (Amy AI Tutor).
 * Isolated output dir. Does not modify second-production assets or validators.
 *
 * Run: pnpm exec node --import tsx/esm ./operations/google-golden-002-production-run.ts
 */
...
export const GOOGLE_G002_PRODUCTION_RUN_PATH = join(
  HERE,
  "..",
  "docs",
  "operations",
  "GOOGLE_GOLDEN_002_PRODUCTION_RUN.md",
);
...
function goldenToContentPackage(script: GoldenScript): ContentPackage {
  ...
  // Tight ~20s VO — golden-002 Amy AI Tutor.
...
function toRenderPackage(videoPath: string, outputDirectory: string): RenderPackage {
  const checksum = createHash("sha256")
    ...
    .slice(0, 24);
  return {
    id: `rp_g002_prod_${checksum}`,
    version: RENDER_PACKAGE_VERSION,
    createdAt: new Date().toISOString(),
    storyboardId: "sb_g002_prod_golden_002",
    assetPackageId: "ap_g002_prod",
    videoPath,
...
function writeReport(state: RunState): string {
  ...
  mkdirSync(dirname(GOOGLE_G002_PRODUCTION_RUN_PATH), { recursive: true });
  writeFileSync(GOOGLE_G002_PRODUCTION_RUN_PATH, md.join("\n"), "utf8");
  return GOOGLE_G002_PRODUCTION_RUN_PATH;
}

export async function runGoogleGolden002Production(): Promise<RunState> {
  ...
  const outRoot = join(REPO_ROOT, ".amynest-assets", "google-production-golden-002");
  ...
  // Reuse Veo raws when present (caption/OCR fixes); set AMYNEST_REUSE_VEO=0 to force regen.
  if (process.env.AMYNEST_REUSE_VEO === undefined) {
    process.env.AMYNEST_REUSE_VEO = "1";
  }
  let clips: string[] = [];
  ...
  // --- Final mux ---
  const finalPath = join(outRoot, "amynest-google-golden-002.mp4");
  try {
    const t0 = Date.now();
    assembleMaster({
      clips,
  ...
  try {
    ...
    try {
      new ContinuousLearningEngine().ingest({
        ...
        campaignByVideoId: { [result.video.videoId]: "google-golden-002" },
        metrics: [
          synthesizeMetricsFromViews({
...
function writeCharacterContinuityReport(
  ...
  const out = join(
    ...
    "docs",
    "operations",
    "CHARACTER_CONTINUITY_REPORT_G002.md",
  );
...
if (isMain) {
  runGoogleGolden002Production()
    .then((state) => {
      console.log(`\nGOOGLE_GOLDEN_002_PRODUCTION_RUN → ${state.status}`);
      console.log(`Report: ${GOOGLE_G002_PRODUCTION_RUN_PATH}`);
