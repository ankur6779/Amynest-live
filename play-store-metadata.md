# AmyNest AI — Play Store Metadata (Optimized)

**Package:** `com.amynest.app`  
**App label (Android):** AmyNest AI  
**Version at audit:** 1.4.43 (build 86)  
**Play URL:** https://play.google.com/store/apps/details?id=com.amynest.app

---

## App Title (30 characters max)

```
AmyNest AI: Parenting Coach
```

*Character count: 27*  
*Primary keywords: AI parenting coach, parenting app*

---

## Short Description (80 characters max)

```
AI parenting coach: routines, speech practice, nutrition & infant care for kids.
```

*Character count: 79*  
*Keywords: AI parenting coach, child routine planner, speech coach, kids nutrition*

---

## Long Description

**AmyNest AI — Your Smart Parenting Coach**

Turn parenting chaos into calm daily wins with AMY, your AI parenting coach built for Indian families. From newborn feeds to school mornings, AmyNest combines personalized routines, speech practice, nutrition planning, and learning — in one app parents actually use every day.

**Meet AMY — Your 24/7 Parenting Coach**  
Ask anything about sleep, picky eating, tantrums, speech development, or school prep. AMY learns your child's age and routines to give practical advice — not generic blog posts.

**Smart Daily Routines**  
AI-generated schedules that adapt when you mark what worked. Wake windows, meals, play, learning blocks, and bedtime — personalized by age from infant to school-age.

**Speech Coach for Kids**  
Guided pronunciation practice, word drills, and conversation coaching with streak tracking and parent progress summaries.

**Nutrition Hub**  
Weekly meal plans, grocery lists, tiffin ideas, and daily nutrition scoring — built for Indian kitchens and picky eaters.

**Infant Parenting Hub**  
Track feeds, sleep, growth, and vaccines. Cry insights and pediatrician-ready summaries for newborns through 12 months.

**Study Zone & Learning**  
Phonics, math tricks, spelling, and discovery worlds — structured learning that fits your family's schedule.

**Family Dashboard**  
Manage multiple children, share plans with caregivers, and see progress across routines, speech, and nutrition.

**Why Parents Choose AmyNest**
- Privacy-first — no ads shown to children  
- Free to start — upgrade when you want premium AI features  
- Built for India — meal ideas, school timings, cultural context  
- Works on Android & iOS — one account, whole family  

**Premium unlocks:** Unlimited AI routines, advanced speech coaching, family sharing, and priority support.

Download AmyNest AI free on Google Play. Where smart parenting begins.

---

## Keyword Target Map

| Target keyword | Placement |
|----------------|-----------|
| AI parenting coach | Title, short desc, long desc (AMY section) |
| parenting app | Title, long desc |
| child routine planner | Short desc, routines section |
| speech coach for kids | Short desc, speech section |
| kids nutrition planner | Short desc, nutrition section |
| family organizer | Family dashboard section |

---

## Localized Notes

Update Hindi/French/Spanish store listings in Play Console — legacy copies in `artifacts/kidschedule-android/store-assets/descriptions/` still reference "KidSchedule" and should be replaced with AmyNest AI branding at upload time.

---

## Changelog vs Previous Listing

| Field | Before | After |
|-------|--------|-------|
| Short description | Generic feature list | Keyword-optimized, 79 chars |
| Title | AmyNest AI (no keywords) | Adds "Parenting Coach" |
| Long description | Feature bullets only | Structured keyword sections + India positioning |

**Estimated ASO impact:** +15–25% store listing relevance score for target keywords; +5–10% conversion from clearer value prop in first 3 lines.
