...
- **Nutrition Hub:** Module with age-group-specific nutrient library, multi-cuisine weekly meal plans, and a daily nutrition score checklist.
...
- **Android WebView Wrapper (`android/`):** Native Android app (package `com.amynest.app`) that wraps `https://amynest.in` in a full-screen WebView. Uses a two-channel push bridge: `window.AndroidPush` (via `WebView.addJavascriptInterface`) exposes synchronous `getPushToken()` and `getPermissionStatus()` methods; `window.onAndroidToken(token)` (called via `WebView.evaluateJavascript`) delivers fresh FCM tokens to the page. `KidScheduleFcmService` shows system-tray notifications when app is backgrounded. `PushBridge` companion holds a `WeakReference<WebView>` for cross-process token delivery. `assetlinks.json` hosted at `kidschedule/public/.well-known/assetlinks.json` enables App Links (SHA-256 fingerprint must be replaced from the release keystore).
...
- **Amy Speech Coach:** `artifacts/kidschedule/src/pages/speech-coach` (web + Capacitor iOS/Android) communication-skills hub backed by the shared `lib/speech-coach` library. Surfaces 9 sub-sections — Dashboard, Milestone Checker, AI Pronunciation Practice, Read Aloud & Repeat, Daily Speech Games, Parent Guidance, Emotion & Confidence Builder, Speech Progress Reports, and Connect with Certified Speech Experts — with age-band-aware content. Pronunciation and read-aloud audio are synthesized via ElevenLabs ("Amy" voice) with content-addressed caching; advanced pronunciation practice is gated by the freemium daily-cap (free users get one logged attempt per day, then 402 Payment Required, mirroring the rest of the Parent Hub paywall). i18n keys live under `screens.speech_coach.*` and are mirrored across web and mobile per the Cross-Platform Sync Rule.
...
**Study Zone and app features live in `artifacts/kidschedule` only** (served on web and via Capacitor). The archived Expo app is not maintained. This applies to:
