...
- **Spelling Mastery (kidschedule, age 2+):** Full learning module under Parent Hub → Activities. Four age-bands (2-4/4-6/6-8/8-10+) × three difficulties (easy/medium/hard) × seven modes (Learn / Practice / Dictation / Competition / **Tournament** / **Battle** / Parent Mode). **Tournament Mode** is a 3-round elimination ladder (Easy → Medium → Hard, 5 words each); R1+R2 require ≥3 correct to advance, R3 always counts; per-round difficulty/word-count/pass-threshold are baked into server-side `TOURNAMENT_ROUND_CONFIG`. Tables: `spelling_tournaments` (token / status / currentRound / rounds jsonb / totalScore / eliminatedAtRound / finalizedAt) + extension to `spelling_sessions` (`parentTournamentToken`). Endpoints: `POST /spelling/tournaments/start` (creates tournament + R1 session), `POST /spelling/tournaments/:token/advance` (finalizes active round, returns next round session OR terminal status), `GET /spelling/tournaments/:token`. Failed-round score is NOT added to `totalScore`. Public `/spelling/sessions/start` rejects mode `"tournament"` — only the tournament endpoint can mint those. **Battle Mode** is turn-based You-vs-AI: client picks an opponent strength (`ai_easy` / `ai_medium` / `ai_hard`), server pre-simulates the AI's per-word verdicts at session start using `simulateAiOpponent(count, opponent, sessionToken)` (mulberry32 PRNG seeded by FNV-1a hash of `${opponent}:${sessionToken}` for full determinism + replay-resistance) and stores them on the session row (`aiOpponent`, `aiResults`, `aiFinalScore`). Each `attempt` response now includes `aiResult: { correct, ms } | null` for that index; `finalize` returns `aiScore` + `winner: "you"|"ai"|"tie"` (both sides scored with the SAME `computeCompetitionScore(correct, durationSec)` formula). Session start refines that `mode === "battle"` ⇔ `opponent` is present. Curated 60-word dataset with syllable + phonetic-chunk + hint metadata at `artifacts/api-server/src/data/spelling-words.ts`; AI fallback via OpenAI gpt-4o-mini (Zod-validated JSON). Audio reuses `/api/tts/synthesize` with client-side `playbackRate=0.65` slow mode. Server-authoritative stars/level/badges (recomputed in a transaction with `INSERT…ON CONFLICT DO NOTHING` + `SELECT … FOR UPDATE` to prevent lost updates) and per-run competition leaderboard. **v2 trust model (Task #201):** Competition + Dictation now use server-verified attempt sessions — the server picks the words, stamps `startedAt`, grades each typed guess (NFKC + lowercase + whitespace-stripped equality), and computes the final competition score from server timestamps at finalize. The client only ever sees an opaque `sessionToken` + per-word audio URLs (`/api/spelling/sessions/:token/audio/:idx.mp3`), never the answer string, syllables, chunks, or hint. Per-word `id` returned in the start payload is ALSO opaque (`w${index}`) — NOT the catalog `word.id` (which is `word.toLowerCase()`) — so a tampered client cannot read answers off the start response. Per-word grading is keyed by integer `wordIndex`, not `id`. Per-word replay is blocked at the DB level (`jsonb_set` with a `NOT (attempts ? key)` guard) AND a `finalized_at IS NULL` guard so a finalize that races ahead of an attempt cannot let the attempt slip in post-finalize (would not be reflected in the leaderboard row); finalize is row-locked + idempotent. **Trust hardening (Task #205):** Tournament `/advance` is implemented as a 4-branch in-tx state machine (`advanceTournamentTxImpl` in `spelling.ts`, exported as `_advanceTournamentTxForTest` for direct integration tests) wrapped in `db.transaction` with `SELECT … FOR UPDATE` on the tournament row. The next-round session row INSERT happens INSIDE the same outer tx so tournament state (`currentRound++`, `rounds[]`) and the session backing it are committed atomically. TTS prewarm runs AFTER the tx commits (writes `audioKeys` back via a separate UPDATE) so the lock isn't held while ElevenLabs streams. The four branches: **(1) Normal flow** — latest session unfinalized AND audioKeys populated → finalize, applyRoundResult, update tournament, INSERT next-round session row. **(2) Audio-pending recovery** — latest session unfinalized AND audioKeys empty → DO NOT finalize (would erroneously apply a 0-attempt round and eliminate the kid since the session has no playable audio); re-prewarm TTS for the SAME session and return it. **(3) Legacy recovery** — latest session finalized AND `tournament.rounds[last].sessionToken === activeSession.sessionToken` → previous /advance applied the round + bumped currentRound but its post-tx createSpellingSession never landed; skip finalize/applyRoundResult (already done) and INSERT the missing next-round session inside the tx. **(4) Drift / inconsistent_state** — latest session finalized but does NOT match `rounds[last].sessionToken` (e.g. session finalized via the public `/sessions/:token/finalize` path without /advance applying it) → refuse with 500 rather than guess and double-allocate. Regression coverage in `spelling.test.ts`: 3 in-tx integration tests (legacy-recovery / audio-pending / drift) drive `_advanceTournamentTxForTest` directly inside `db.transaction` so every branch is exercised end-to-end including DB state assertions on the post-tx tournament + session rows. `POST /spelling/progress` is now restricted to `source: "parent"` ONLY — the parent literally taps ✓/✗ in Parent Mode, so the assertion is out-of-band of the tampered-client surface. Learn + Practice used to write here too but were trivially scriptable (a kid in Chrome devtools could post `correct: true` repeatedly and inflate stars / level / badges); they're now UI-only flows that don't accumulate progress. Star accumulation happens exclusively via Parent Mode + the server-graded session flow (Dictation / Competition / Tournament / Battle). The legacy `POST /spelling/competition/score` endpoint has been removed — the leaderboard row is now written exclusively inside session finalize. Tables: `spelling_progress`, `spelling_competition_scores`, `spelling_sessions` (jsonb words/audioKeys/attempts + finalScore/finalDurationSec/finalWordsAttempted/finalWordsCorrect/competitionScoreId). Routes under `/api/spelling/*`. UI in `artifacts/kidschedule/src/components/spelling-mastery.tsx` + `src/hooks/use-spelling.ts` (`useSpellingSession` for v2, `useSpellingProgress.recordAttempt(correct, source)` for legacy). Pure-logic tests for `normaliseSpellingGuess` + `computeCompetitionScore` + `applyAttempt` live in `artifacts/api-server/src/routes/spelling.test.ts`. **Parent Hub placement:** Spelling Mastery is its own top-level section (id `spelling-mastery`, gate `hub_spelling`, bands `2-4` through `12-15`, render guard `totalAgeMonths >= 12 && < 180`) — sits right after Phonics in the section grid. The gate id `hub_spelling` is registered in `PARENT_HUB_FEATURES` in `artifacts/api-server/src/services/featureUsageService.ts`.
