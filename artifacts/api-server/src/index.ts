...
async function startBackgroundTasks(): Promise<void> {
  ...
  if (isModuleEnabled("crons") && isNotificationCronEnabled()) {
    ...
    await runBackgroundPhase("phonics_curriculum_cron", async () => {
      const { startPhonicsCurriculumCron } = await import("./lib/phonicsCurriculumCron.js");
      startPhonicsCurriculumCron();
    });
    await runBackgroundPhase("phonics_content_drip_cron", async () => {
      const { startPhonicsContentDripCron } = await import("./lib/phonicsContentDripCron.js");
      startPhonicsContentDripCron();
    });
    ...
    await runBackgroundPhase("tts_orphan_cleanup_cron", async () => {
      const { startTtsOrphanCleanupCron } = await import("./lib/ttsOrphanCleanupCron.js");
      startTtsOrphanCleanupCron();
    });
