...
export async function purgeChildScopedData(
  ...
  const intChildDeletes: Array<{ table: string; run: () => Promise<{ id?: unknown }[]> }> = [
    ...
    { table: "phonics_v3_mastery", run: () => tx.delete(phonicsV3MasteryTable).where(eq(phonicsV3MasteryTable.childId, childId)).returning({ id: phonicsV3MasteryTable.id }) },
    { table: "phonics_v3_fluency", run: () => tx.delete(phonicsV3FluencyTable).where(eq(phonicsV3FluencyTable.childId, childId)).returning({ id: phonicsV3FluencyTable.id }) },
    { table: "phonics_v3_story_progress", run: () => tx.delete(phonicsV3StoryProgressTable).where(eq(phonicsV3StoryProgressTable.childId, childId)).returning({ id: phonicsV3StoryProgressTable.id }) },
    { table: "phonics_v3_missions", run: () => tx.delete(phonicsV3MissionsTable).where(eq(phonicsV3MissionsTable.childId, childId)).returning({ id: phonicsV3MissionsTable.id }) },
    { table: "phonics_v3_retention", run: () => tx.delete(phonicsV3RetentionTable).where(eq(phonicsV3RetentionTable.childId, childId)).returning({ id: phonicsV3RetentionTable.id }) },
