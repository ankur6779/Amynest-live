 ...
 *
 * Paid *subscribers* (RevenueCat / Razorpay / Stripe with an active paid period)
 * receive shareable gift tokens. Everyone else — free, bonus-only, trials, grace —
 * receives personal bonus premium days. Using isPremiumNow here previously treated
 * active bonusExpiresAt as "paid", minting self-owned gifts that self_redeem blocks.
 */
export async function tryGrantReferralReward(referrerUserId: string): Promise<number> {
  ...
  if (granted > 0) {
    await notifyReferralRewardUnlocked(
      referrerUserId,
      granted,
      already + granted,
      isPaidSubscriber,
