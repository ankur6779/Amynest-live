...
export async function ensurePhonicsCurriculumTables(): Promise<void> {
  ...
  await ensurePhonicsV3ProgressTables();
...
/** Phonics V3 Elite — mastery, fluency, stories, missions server persistence. */
export async function ensurePhonicsV3ProgressTables(): Promise<void> {
  const ddl = `
  ...
  logger.info(
    { evt: "db.ensure", table: "phonics_v3_progress" },
    "Ensured phonics V3 progress tables",
  );
}
