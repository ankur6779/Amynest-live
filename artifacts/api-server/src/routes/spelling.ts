...
router.post("/spelling/progress", async (req, res): Promise<void> => {
  ...
  //
  // Race-safety: we first run an idempotent INSERT…ON CONFLICT DO NOTHING
  // to guarantee the row exists, then SELECT … FOR UPDATE to take a row
  // lock for the rest of the txn. Concurrent attempts on the same
  // (childId, ageGroup) serialize behind that lock instead of clobbering
  ...
  const updated = await db.transaction(async (tx) => {
    ...
    // existing[0] is guaranteed by the upsert above. Defensive fallback
    // keeps the type narrow if the insert was somehow skipped.
...
router.post("/spelling/competition/score", async (req, res): Promise<void> => {
  ...
  // Sanity bound: a real human can't legitimately type+submit a spelling
  // word in under ~1 second. Reject obviously impossible runs to keep
  // the leaderboard from being trivially gamed by a tampered client.
  // (v2 will move to per-attempt server-signed tokens — see follow-up.)
  if (durationSec < wordsAttempted) {
    res.status(400).json({ error: "duration_too_short" });
    return;
  }
  if (!(await ownsChild(userId, childId))) {
    res.status(403).json({ error: "forbidden" });
