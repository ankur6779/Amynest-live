        ...
        <View style={{ padding: 12, paddingTop: 0, gap: 10 }}>
          ...
          {!practiceMode && (
            <View style={{ flexDirection: "row", gap: 8 }}>
              ...
              <Pressable onPress={onStar} style={[styles.starBtn, starred && { backgroundColor: "rgba(251,191,36,0.2)", borderColor: "#fbbf24" }]}>
                <Text style={{ fontSize: 14, color: starred ? "#fbbf24" : "rgba(255,255,255,0.4)" }}>{starred ? "⭐" : "☆"}</Text>
              </Pressable>
            </View>
          )}
            ...
            <View style={{ gap: 8 }}>
              ...
              <View style={styles.optGrid}>
                {trick.practiceQ.options.map(opt => {
                  ...
                  if (submitted) {
                    if (isC) { bg = "rgba(34,197,94,0.2)"; border = "#22c55e"; }
                    else if (isSel && !isC) { bg = "rgba(239,68,68,0.2)"; border = "#ef4444"; }
              ...
              {!submitted ? (
                ...
                <View style={{ gap: 6 }}>
                  <View style={[styles.feedback, { backgroundColor: isCorrect ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)" }]}>
                    <Text style={{ color: isCorrect ? "#4ade80" : "#f87171", fontSize: 12, fontWeight: "700", textAlign: "center" }}>
  ...
  return (
    <View style={{ gap: 8 }}>
      <View style={styles.statsRow}>
        ...
        <Text style={[styles.tabHint, { color: "#fbbf24", fontWeight: "700" }]}>⭐ {mastered}/{pool.length} mastered</Text>
      </View>
      {pool.map(t => (
        <TrickCard
          key={t.id}
    ...
    return (
      <View style={{ alignItems: "center", paddingVertical: 24, gap: 12 }}>
        ...
        <View style={[styles.scorePill, { backgroundColor: pct === 100 ? "rgba(251,191,36,0.2)" : "rgba(148,163,184,0.2)" }]}>
          <Text style={[styles.scoreText, { color: pct === 100 ? "#fbbf24" : c.textMuted }]}>
        ...
        <Pressable onPress={handleRestart} style={[styles.submitBtn, { backgroundColor: "#f59e0b", paddingHorizontal: 32 }]}>
    ...
    <View style={{ gap: 10 }}>
      ...
      <View style={styles.optGrid}>
        {cur.practiceQ.options.map(opt => {
          ...
          if (submitted) {
            if (isC) { bg = "rgba(34,197,94,0.2)"; border = "#22c55e"; }
            else if (isSel) { bg = "rgba(239,68,68,0.2)"; border = "#ef4444"; }
            else { color = c.textDim; }
      ...
      {!submitted ? (
        <Pressable onPress={handleSubmit} disabled={!selected} style={[styles.submitBtn, { backgroundColor: "#f59e0b" }, !selected && { opacity: 0.3 }]}>
          <Text style={styles.submitText}>Check Answer ✓</Text>
        </Pressable>
      ) : (
        <Pressable onPress={handleNext} style={[styles.submitBtn, { backgroundColor: idx + 1 >= SESSION_SIZE ? "#f59e0b" : "#6366f1" }]}>
