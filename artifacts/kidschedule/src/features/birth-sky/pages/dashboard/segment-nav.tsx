 ...
 * Internal segment ids unchanged (sky / astronomy / tradition / reflect).
 */
...
const TABS: {
  ...
  { id: "sky", label: "Home" },
  { id: "astronomy", label: "Sky Map" },
  { id: "tradition", label: "Kundli" },
...
export function BirthSkySegmentNav({ active, onChange }: Props) {
  ...
  return (
    <div
      role="tablist"
      aria-label={`${AMY_ASTRO_PRODUCT_SHORT} sections`}
      ...
      {TABS.map((tab, i) => {
        ...
        return (
          <button
            ...
            className={cn(
              "min-h-11 flex-1 rounded-xl px-2 text-xs font-bold tracking-wide transition-colors",
              selected
