...
export function useSubscription() {
  ...
  useEffect(() => {
    // Placeholder / UI-only FREE must never purge Learning Zone data.
    // Settled free only clears regenerable study batches + audio caches —
    // never phonics mastery / offline sync queues (see learning-zone-premium-cache).
    if (
      shouldClearLearningZonePremiumCaches({
