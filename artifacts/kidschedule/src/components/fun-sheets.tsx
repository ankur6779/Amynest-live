...
export function FunSheets({
    ...
    async (targetPage: number, signal?: AbortSignal) => {
      ...
      try {
        const res = await authFetch(
          `/api/funsheets/list?childId=${numericChildId}&page=${targetPage}`,
          { signal },
        );
        if (!res.ok) {
  ...
  return (
    <div className="space-y-4" data-testid="fun-sheets-section">
      {/* Daily quota banner */}
      {quota && (
        <div
          data-testid="funsheet-quota-banner"
          className={[
            ...
            quotaExhausted
              ...
              : "bg-teal-50/70 dark:bg-teal-500/10 border border-teal-200 dark:border-teal-500/30 text-teal-900 dark:text-teal-200",
          ].join(" ")}
        >
        ...
        <Card className="bg-white/60 dark:bg-white/[0.04] border border-white/50 dark:border-white/10 rounded-2xl">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {childName} has downloaded every Fun Sheet in the library. We add new
              ones regularly — check back soon!
            </p>
          </CardContent>
        </Card>
      )}
          ...
          <div className="grid grid-cols-2 gap-3" data-testid="funsheet-grid">
            {files.map((file) => {
              ...
              return (
                <div
                  key={file.id}
                  data-testid={`funsheet-card-${file.id}`}
                  className={[
                    "group relative flex flex-col rounded-2xl overflow-hidden border transition-all duration-200",
                    "bg-white/70 dark:bg-white/[0.04] backdrop-blur-xl",
                    file.downloaded
                      ? "border-teal-200/60 dark:border-teal-400/20 opacity-70"
                      : "border-white/60 dark:border-white/10 hover:border-teal-300/60",
                  ].join(" ")}
                >
                  {/* Thumbnail */}
                  ...
                  <div className="flex flex-col gap-2 p-2.5">
                    <p
                      className="text-[12px] font-semibold text-foreground leading-snug line-clamp-2"
                      title={file.name}
                    >
                      {file.name}
                    </p>
                    ...
                    <div className="flex gap-1.5">
                      <Button
                        ...
                        size="sm"
                        onClick={() => setPreviewing(file)}
                        className="flex-1 rounded-xl text-[11px] h-7 gap-1 px-2"
                        data-testid={`funsheet-preview-btn-${file.id}`}
                      >
                      ...
                      <Button
                        ...
                        className={[
                          "flex-1 rounded-xl text-[11px] h-7 gap-1 px-2",
                          file.downloaded
                            ? "bg-teal-500/20 text-teal-700 dark:text-teal-300 border border-teal-300/60 hover:bg-teal-500/30"
                            : "bg-teal-600 hover:bg-teal-700 text-white",
                        ].join(" ")}
                        data-testid={`funsheet-download-btn-${file.id}`}
                      >
                        {isDownloading ? (
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className="flex items-center justify-center gap-3 pt-1">
              <Button
                ...
                data-testid="funsheet-prev-button"
              ...
              <Badge
                variant="outline"
                data-testid="funsheet-page-indicator"
                className="rounded-full px-3 py-1 font-bold text-[11px]"
              >
                Page {pagination.page + 1} of {pagination.totalPages}
                <span className="text-muted-foreground ml-1.5">· {pagination.total} sheets</span>
              </Badge>
              <Button
                variant="outline"
                size="sm"
                ...
                data-testid="funsheet-next-button"
                className="gap-1 rounded-xl"
              >
                Next
      ...
      >
        <DialogContent
          ...
          data-testid="funsheet-preview-dialog"
        >
...
function ThumbnailWithFallback({ src, alt }: { src: string; alt: string }) {
  ...
  if (errored) {
    return (
      <div className="flex flex-col items-center justify-center text-teal-400 dark:text-teal-300 p-4 w-full h-full">
