...
import { Compass, Sparkles, Lightbulb, CheckCircle2, SkipForward, Languages } from "lucide-react";
import { type LifeSkillTask, type LifeSkillCategory, type LifeSkillLang, type CategoryStat, ageBandForLifeSkills, ageBandLabel, CATEGORY_EMOJI, CATEGORY_LABEL, DIFFICULTY_LABEL, POINTS_BY_DIFFICULTY, pickDailyLifeSkillTasks, tasksFor, buildAmyLifeSkillInsight, uiLabel } from "@workspace/life-skills";
...
function loadStats(childId: string | number, fallbackLang: LifeSkillLang): ChildLifeSkillStats {
  ...
  try {
    ...
    const cats: LifeSkillCategory[] = ["hygiene", "social", "responsibility", "emotional", "money", "time", "self_care", "chores"];
    const byCategory = {
      ...def.byCategory
    };
    for (const c of cats) {
    ...
    const lang: LifeSkillLang = p.lang === "hi" || p.lang === "hinglish" || p.lang === "en" ? p.lang : fallbackLang;
    return {
...
export function LifeSkillsZone({
  child
}: LifeSkillsZoneProps) {
  ...
  useEffect(() => {
    setStatsState(loadStats(child.id, fallbackLang)); /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [child.id]);
  ...
  const setLang = (lang: LifeSkillLang) => updateStats(prev => ({
    ...prev,
    lang
  }));
  ...
  const todaysTasks = useMemo(() => pickDailyLifeSkillTasks({
    ageBand,
    date,
    childKey: child.id,
    count: 2,
    previousIds: yesterdayPicks
  ...
  const lockedTasks = useMemo<LifeSkillTask[]>(() => {
    ...
    const resolved = todayRec.taskIds.map(id => todaysTasks.find(t => t.id === id) ?? tasksFor(ageBand).find(t => t.id === id)).filter((t): t is LifeSkillTask => Boolean(t));
  ...
  const handleAction = (task: LifeSkillTask, action: "done" | "skip") => {
    updateStats(prev => {
      ...
      const baseRec: DailyRecord = prev.daily[date] ?? {
        taskIds: lockedIds,
        done: [],
        skipped: []
      };
      ...
      const byCategory: Record<LifeSkillCategory, CategoryStat> = {
        ...prev.byCategory
      };
      byCategory[cat] = {
  ...
  const remainingTasks = lockedTasks.filter(t => !(stats.daily[date]?.done ?? []).includes(t.id) && !(stats.daily[date]?.skipped ?? []).includes(t.id));
  ...
  return <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
        ...
        <span className="ml-auto inline-flex items-center gap-1 rounded-full border px-1 py-0.5">
          ...
          {langs.map(l => <button key={l} onClick={() => setLang(l)} className={`px-1.5 rounded-full text-[11px] ${lang === l ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          return <SubItemGate key={task.id} sectionId="hub_life_skills" subItemId={task.id}>
              <Card className={settled ? "opacity-70" : ""}>
                <CardContent className="p-3 space-y-2">
                  ...
                  {!settled && <div className="flex gap-2">
