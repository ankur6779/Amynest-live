...
const CATEGORY_META: Record<MCategory, {
  label: string;
  emoji: string;
  color: string;
  bg: string;
...
// ── 12–24 MONTHS ────────────────────────────────────────────────────────
{
  ...
  id: "b1224_one_step",
  emoji: "🎯",
  title: "Follows One-Step Commands",
  category: "language",
...
const ENCOURAGE_NOT_STARTED = ["Let's start when you're both ready 💜", "No pressure — every baby moves at their own pace.", "Pick a calm moment to try this together."];
const ENCOURAGE_IN_PROGRESS = ["Great progress! Keep going 🌟", "Almost there — consistency wins.", "You're doing wonderfully. Try again today."];
const ENCOURAGE_ACHIEVED = ["Amazing! You unlocked this together 🏆", "Look at that growth! Celebrate this win.", "Beautifully done — this is real progress."];
const PARENT_TIPS = ["Don't worry if your baby is slightly behind — milestone ranges are wide and very normal.", "Every baby develops at their own pace. Comparison is the thief of parenting joy.", "Some weeks are big leaps, others are quiet. Both are part of growth.", "If a milestone feels stuck for 2+ months past the upper age, mention it at your next check-up — early support helps when needed.", "Skills don't appear in a clean order — your baby may skip crawling and go straight to walking. That's also normal.", "Babies often regress briefly before a big leap. A 'bad' week is sometimes a sign growth is coming."];
...
export function BuddyMilestonePlanner({
  ...
  const bandMilestones = useMemo(() => MILESTONES.filter(m => {
    ...
    const [bMin, bMax] = ageBand === "0-3" ? [0, 3] : ageBand === "3-6" ? [3, 6] : ageBand === "6-12" ? [6, 12] : [12, 24];
  ...
  const stats = useMemo(() => {
    ...
    const nextFocus = bandMilestones.filter(m => progress[m.id]?.state !== "achieved").slice(0, 2).map(m => m.title);
    return {
      total,
      done,
      ip,
      ...
      <div>
        ...
        <div className="space-y-2.5">
          ...
          {weeklyPlan.length === 0 && <div className="rounded-xl border-2 border-dashed border-border/60 px-3 py-6 text-center">
              ...
              <p className="text-[12px] text-muted-foreground">{t("components.infant_milestones.no_milestones_for_this_exact_age_yet_check_back_soon")}</p>
            </div>}
        </div>

        {/* Extra opened — when user clicked an item from "All for this age" */}
        {extraOpened && <div className="mt-3 space-y-1.5">
            <div className="flex items-center gap-1.5">
              <BookOpen className="h-3 w-3 text-primary" />
              <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{t("components.infant_milestones.also_exploring")}</p>
            </div>
            <MilestoneCard milestone={extraOpened} state={progress[extraOpened.id]?.state ?? "not_started"} isOpen onToggle={() => setExpanded(null)} onSetState={s => setState(extraOpened.id, s)} />
          </div>}
      </div>
...
function ActionBtn({
  ...
  return <button onClick={onClick} className={`${base} ${cls}`}>
...
function BandLibrary({
  ...
  return <div className="rounded-xl bg-white/40 dark:bg-white/[0.03] border border-border overflow-hidden">
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center justify-between gap-2 px-3 py-2.5 text-left">
        <div className="flex items-center gap-2">
          <TrendingUp className="h-3.5 w-3.5 text-primary" />
          <span className="text-xs font-bold text-foreground">{t("components.infant_milestones.all_for_this_age")}</span>
          <span className="text-[10px] text-muted-foreground">({others.length} {t("components.infant_milestones.more")}</span>
        </div>
      ...
      {open && <div className="px-3 pb-3 space-y-1.5">
        ...
        const cat = CATEGORY_META[m.category];
        return <button key={m.id} onClick={() => setExpanded(m.id)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg bg-white/60 dark:bg-white/5 border border-border hover:border-primary/40 transition-all text-left">
