...
export function RoutineInlineMeals({
  ...
  return <div className="mt-2.5 rounded-xl border border-border dark:border-border bg-muted dark:bg-card overflow-hidden">
      ...
      scrollbarWidth: "none"
          ...
          {meals.map(m => <MiniMealCard key={m.id} meal={m} onOpen={e => {
        e.stopPropagation();
        setOpenMeal(m);
      }} />)}
        </div>}

      {openMeal && createPortal(<RecipeSheet meal={openMeal} onClose={handleClose} />, document.body)}
    </div>;
}
...
function MiniMealCard({
  ...
  return <button onClick={onOpen} className="group shrink-0 w-[130px] rounded-xl overflow-hidden border border-border bg-card hover:border-border dark:hover:border-primary hover:shadow-sm transition-all text-left">
      ...
      background: `linear-gradient(135deg, ${meal.bgGradient[0]}, ${meal.bgGradient[1]})`
        ...
        {meal.tags[0] && <span className="absolute bottom-1 left-1.5 text-[9px] font-bold bg-white/80 text-foreground px-1.5 py-0.5 rounded-full">
            {meal.tags[0]}
          </span>}
      </div>
...
function RecipeSheet({
  ...
  return <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-card rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg max-h-[88vh] overflow-y-auto shadow-2xl animate-in slide-in-from-bottom duration-300" onClick={e => e.stopPropagation()}>
        <div className="relative h-[160px] flex items-center justify-center text-[88px] rounded-t-3xl" style={{
        background: `linear-gradient(135deg, ${meal.bgGradient[0]}, ${meal.bgGradient[1]})`
      }}>
          <button onClick={onClose} className="absolute top-3 right-3 h-8 w-8 rounded-full bg-white/85 hover:bg-white text-foreground flex items-center justify-center shadow">
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="p-5 space-y-4">
          ...
          <button onClick={onClose} className="w-full py-3 rounded-2xl border border-border text-foreground text-sm font-bold hover:bg-muted/50 transition-colors">
            {t("components.routine_inline_meals.close")}
          </button>
        </div>
      </div>
    </div>;
