...
function getWakeSpec(months: number): WakeWindowSpec {
  ...
  if (months < 2) return {
    ...
    totalDayMin: 360,
    napDurMin: 60,
    nightSleepHrs: "8–10 (interrupted)"
  };
  if (months < 3) return {
    range: "2–3 mo",
    windowMin: 90,
...
export function WakeWindowSystem({
  ...
  if (!lastEvent) status = "idle";else if (lastEvent.type === "down") status = "asleep";else {
    const earlyTired = spec.windowMin - 15; // 15 min before min window = entering window
    if (elapsedMin < earlyTired) status = "normal";else if (elapsedMin < spec.windowMax) status = "tired";else status = "overtired";
  }
  const remainingToOptimal = Math.max(0, spec.windowMin - elapsedMin);
  const progressPct = isAwake ? Math.min(100, Math.round(elapsedMin / spec.windowMax * 100)) : 0;
  const STATUS_META: Record<typeof status, {
    label: string;
    color: string;
    bg: string;
    emoji: string;
    ...
    idle: {
      ...
      bg: "bg-muted/40",
      emoji: "💤",
      msg: "Tap 'Just woke up' below to start tracking the wake window."
    },
    asleep: {
      label: "Asleep",
      ...
      <div className={`rounded-2xl ${m.bg} border-2 border-current ${m.color} p-4 backdrop-blur-md shadow-[0_8px_24px_-12px_rgba(99,102,241,0.4)]`}>
        <div className="flex items-center justify-between gap-2 mb-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider opacity-80">{t("components.infant_sleep_module.status")}</p>
              <p className="text-base font-bold">{m.label}</p>
            </div>
          </div>
          {isAwake && <div className="text-right">
              <p className="text-[10px] font-bold uppercase tracking-wide opacity-80">{t("components.infant_sleep_module.awake_for")}</p>
              <p className="text-lg font-bold tabular-nums">{fmtDuration(elapsedMin)}</p>
            </div>}
          {status === "asleep" && lastEvent && <div className="text-right">
              <p className="text-[10px] font-bold uppercase tracking-wide opacity-80">{t("components.infant_sleep_module.asleep_for")}</p>
              <p className="text-lg font-bold tabular-nums">{fmtDuration(elapsedMin)}</p>
            </div>}
        </div>
        ...
        {isAwake && <>
            <div className="h-2 rounded-full bg-white/40 dark:bg-white/10 overflow-hidden mb-2">
              <div className="h-full rounded-full bg-current transition-all duration-700" style={{
            width: `${progressPct}%`
          }} />
            </div>
            <div className="flex justify-between text-[10px] opacity-80">
              <span>0</span>
              <span className="font-bold">{t("components.infant_sleep_module.optimal")} {spec.windowMin}–{spec.windowMax} {t("components.infant_sleep_module.min")}</span>
...
function TodaySnapshot({
  ...
  return <div className="rounded-xl bg-white/50 dark:bg-white/[0.03] border border-border overflow-hidden">
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center justify-between gap-2 px-3 py-2.5 text-left">
        <div className="flex items-center gap-2">
          <Activity className="h-3.5 w-3.5 text-primary" />
          <span className="text-xs font-bold text-foreground">{t("components.infant_sleep_module.today")}</span>
          <span className="text-[10px] text-muted-foreground">
            · {napCount} {t("components.infant_sleep_module.nap")}{napCount === 1 ? "" : "s"} · {fmtDuration(Math.round(todaySleepMin))}
          </span>
          <span className="text-[10px] text-muted-foreground ml-1">{t("components.infant_sleep_module.target")} {fmtDuration(spec.totalDayMin)}</span>
        </div>
...
export function SleepIssueDetector({
  ...
  return <div className="space-y-2">
      ...
      return <div key={iss.id} className={`rounded-xl ${m.bg} border ${m.border} p-3`}>
...
function generateRoutine(months: number, wakeUpTime = "7:00 AM"): RoutineItem[] {
  ...
  for (let i = 0; i < napCount; i++) {
    ...
    if (i === 0 && months >= 6) items.push({
      id: `meal1`,
      time: fmt(new Date(cur.getTime() + 30 * 60_000)),
      activity: "Breakfast / Solids",
      emoji: "🥣"
    });
    if (hr >= 11 && hr <= 13 && months >= 6) {
      const lunch = new Date(cur);
...
export function RoutineBuilder({
  ...
  const regenerate = () => {
    ...
    persist(fresh);
    toast({
      description: t("toasts.infant_sleep.routine_regenerated", {
        name: childName
      })
    });
  };
  ...
  const saveEdit = (id: string) => {
    ...
    setEditing(null);
    toast({
      description: t("toasts.infant_sleep.saved")
    });
  };
            ...
            {editing === item.id ? <div className="p-3 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setEditing(null)} className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-muted text-foreground text-[11px] font-bold">
                    <X className="h-3 w-3" /> {t("components.infant_sleep_module.cancel")}
                  </button>
                </div>
              </div> : <div className="flex items-center gap-2 px-3 py-2">
...
export function SleepWeeklyInsights({
  ...
  const stats = useMemo(() => {
    ...
    const trend: "good" | "low" | "no_data" = dailyTotals.length === 0 ? "no_data" : avgMin < expectedDayTotal * 0.8 ? "low" : "good";
    return {
      dailyTotals,
      avgMin,
      napCount,
  ...
  if (stats.trend === "no_data") {
    return <div className="rounded-xl bg-muted/40 border border-border p-3 text-center">
        <p className="text-[12px] text-muted-foreground">
          {t("components.infant_sleep_module.log_a_few_wake_ups_and_naps_for_at_least_2_3_days_to_see_wee")}
        </p>
      </div>;
  }
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    barData.push({
      day: ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"][d.getDay() === 0 ? 6 : d.getDay() - 1],
