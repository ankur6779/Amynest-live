 ...
 * Audio playback uses ElevenLabs TTS via /api/tts/synthesize — full poem lines are
 * narrated (server-cached by content hash). Instrumental pack MP3s are not used
 * for playback (they remain in the offline pack for future use).
 *
 * Composition:
...
export function InfantPoems({
  ...
  return <div className="space-y-3" data-testid="infant-poems-section">
      ...
      <div className="grid grid-cols-2 gap-2.5" data-testid="poem-tile-grid">
        ...
        // The tile is "active" any time the fullscreen player is open for
      ...
      {visiblePoems.length === 0 && <div className="rounded-xl border-2 border-dashed border-border/60 px-3 py-6 text-center">
          <Sparkles className="h-5 w-5 text-muted-foreground mx-auto mb-2" />
          <p className="text-[12px] text-muted-foreground">{t("components.infant_poems.more_poems_coming_soon_for_this_age")}</p>
        </div>}
      ...
      <div className="relative z-10 h-full flex flex-col justify-between">
        <div className="flex items-start justify-between">
          ...
          {isLoading ? <span className="text-[9px] font-bold uppercase tracking-wider bg-white/25 backdrop-blur-sm px-1.5 py-0.5 rounded-full inline-flex items-center gap-1">
              ...
              {t("components.infant_poems.loading")}
            </span> : isPlaying ? <span className="text-[9px] font-bold uppercase tracking-wider bg-white/25 backdrop-blur-sm px-1.5 py-0.5 rounded-full">
              {t("components.infant_poems.playing")}
            </span> : null}
        </div>
        <div>
...
function PoemFullscreenPlayer({
  ...
  return createPortal(<AnimatePresence>
      {open && poem && <motion.div ref={containerRef} initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} exit={{
      ...
      duration: 0.25
    }} className="fixed inset-0 z-[100] flex items-stretch justify-center" role="dialog" aria-modal="true" aria-label={`Immersive player for ${poem.title}`} data-testid="poem-fullscreen-player">
          {/* Dark backdrop with poem-specific tint */}
          <motion.div key={poem.id} initial={{
        opacity: 0
      }} animate={{
        opacity: player.fadeInProgress ? 0.65 : 1
      }} exit={{
        opacity: 0
      }} transition={{
        duration: 0.6,
        ...
        backgroundBlendMode: "soft-light"
          ...
          <div className="relative w-full max-w-md mx-auto flex flex-col px-5 py-6 text-white">
            ...
            <div className="flex items-center justify-between mb-4">
              <button ref={closeBtnRef} onClick={onClose} aria-label={t("components.infant_poems.close_immersive_player")} data-testid="poem-fullscreen-close" className="h-10 w-10 rounded-full bg-white/15 hover:bg-white/25 backdrop-blur-md flex items-center justify-center">
                <ChevronDown className="h-5 w-5" />
              </button>
              {player.remainingMs !== null && player.remainingMs > 0 && <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/15 backdrop-blur-md text-xs font-bold">
            ...
            <div className="mb-5 rounded-2xl bg-white/10 backdrop-blur-md p-4 border border-white/15">
              {poem.lines.map((line, i) => <p key={i} className="text-[14px] leading-relaxed text-white/85 text-center">
                  {line}
                </p>)}
            </div>
            ...
            <div className="flex items-center justify-center gap-3 mb-4">
              <ControlButton onClick={() => player.setLoop(!player.loop)} label={player.loop ? "Loop on" : "Loop off"} active={player.loop} testid="poem-loop-toggle">
            ...
            if (!player.isPlaying) {
              ...
              <ControlButton onClick={() => {/* placeholder slot to keep visual symmetry */}} label="Volume" active={false} testid="poem-volume-icon" disabled>
            ...
            {/* Volume slider */}
            <div className="mb-4">
              <label htmlFor="poem-volume" className="sr-only">{t("components.infant_poems.volume")}</label>
              <input id="poem-volume" type="range" min={0} max={1} step={0.05} value={player.volume} onChange={e => player.setVolume(Number(e.target.value))} data-testid="poem-volume-slider" className="w-full accent-white" />
            </div>
...
function ControlButton({
  ...
  return <button onClick={onClick} disabled={disabled} aria-label={label} aria-pressed={active} data-testid={testid} className={`h-12 w-12 rounded-full flex items-center justify-center backdrop-blur-md transition-all ${disabled ? "bg-white/5 text-white/30 cursor-default" : active ? "bg-white/30 text-white" : "bg-white/10 hover:bg-white/20 text-white/80"}`}>
      {children}
    </button>;
}
function GlowingOrb({
  ...
  return <div className="relative h-44 w-44 flex items-center justify-center" aria-hidden>
      ...
      {/* Body */}
      <motion.div className="h-32 w-32 rounded-full" style={{
