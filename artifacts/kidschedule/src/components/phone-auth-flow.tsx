...
export default function PhoneAuthFlow({
  ...
  if (step === "phone" || step === "sending") {
    ...
    return <>
        ...
        textAlign: "left"
        ...
        }}>{t("components.phone_auth_flow.enter_your_mobile_number")}</label>
          <div style={{
          display: "flex",
          gap: "8px"
        }}>
            ...
            <input type="tel" value={phone} onChange={e => setPhone(e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="98765 43210" style={{
            ...inputStyle,
            flex: 1
          ...
          gap: "8px"
            ...
            cursor: "pointer",
            fontFamily: "inherit"
          }}>{t("components.phone_auth_flow.cancel")}</button>
            <button type="button" onClick={() => sendOtp()} disabled={!canSend} style={primaryBtn(!canSend)}>
  ...
  if (step === "otp" || step === "verifying") {
    ...
    return <div style={{
      ...
      textAlign: "left"
        ...
        <input type="tel" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="• • • • • •" maxLength={6} autoFocus style={{
        ...inputStyle,
        textAlign: "center",
        fontSize: "24px",
        letterSpacing: "10px"
      }} />
        <button type="button" onClick={verifyOtp} disabled={!canVerify} style={{
        ...primaryBtn(!canVerify),
        flex: "unset",
        width: "100%"
      }}>
