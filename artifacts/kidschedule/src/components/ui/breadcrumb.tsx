...
const Breadcrumb = React.forwardRef<HTMLElement, React.ComponentPropsWithoutRef<"nav"> & {
  ...
  return <nav ref={ref} aria-label={t("components.ui.breadcrumb.breadcrumb")} {...props} />;
});
...
const BreadcrumbSeparator = ({
  ...
  className,
  ...props
}: React.ComponentProps<"li">) => <li role="presentation" aria-hidden="true" className={cn("[&>svg]:w-3.5 [&>svg]:h-3.5", className)} {...props}>
...
const BreadcrumbEllipsis = ({
  ...
  return <span role="presentation" aria-hidden="true" className={cn("flex h-9 w-9 items-center justify-center", className)} {...props}>
    ...
    <span className="sr-only">{t("components.ui.breadcrumb.more")}</span>
  </span>;
};
