...
import { setProductionCookie } from "@/lib/production-cookie";
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback((value: boolean | ((value: boolean) => boolean)) => {
    ...
    setProductionCookie(SIDEBAR_COOKIE_NAME, String(openState), SIDEBAR_COOKIE_MAX_AGE);
  }, [setOpenProp, open]);
    ...
    return <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        "--sidebar-width": SIDEBAR_WIDTH_MOBILE
          ...
          <SheetHeader className="sr-only">
            <SheetTitle>{t("components.ui.sidebar.sidebar")}</SheetTitle>
            <SheetDescription>{t("components.ui.sidebar.displays_the_mobile_sidebar")}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return <Button data-sidebar="trigger" data-slot="sidebar-trigger" variant="ghost" size="icon" className={cn("h-7 w-7", className)} onClick={event => {
    ...
    toggleSidebar();
      ...
      <PanelLeftIcon />
      <span className="sr-only">{t("components.ui.sidebar.toggle_sidebar")}</span>
    </Button>;
}
