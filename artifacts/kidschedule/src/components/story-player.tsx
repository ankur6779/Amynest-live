...
interface StoryFlowPlayerProps {
  ...
  onEnded: () => void;
  onError: () => void;
  onProgress: (storyId: number, positionSec: number, options?: {
    durationSec?: number;
    completed?: boolean;
...
export function StoryFlowPlayer({
  ...
  return <div data-on-dark className="relative overflow-hidden rounded-2xl bg-black shadow-2xl shadow" data-testid={`story-flow-player-${story.id}`}>
      ...
      <div className="absolute left-0 right-0 top-0 z-20 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1">
            {Array.from({
            length: Math.min(totalStories, 20)
          }).map((_, i) => <div key={i} className={`h-1 rounded-full transition-all ${i === storyIndex % 20 ? "w-6 bg-white" : i < storyIndex % 20 ? "w-3 bg-white/40" : "w-3 bg-white/20"}`} />)}
          </div>
      ...
      {autoAdvanceIn !== null && !showLoopBanner && <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-4 bg-black/75 backdrop-blur-sm">
          <p className="text-sm text-white/70">{t("components.story_player.up_next")}</p>
          <p className="text-2xl font-bold text-white">
            {t("components.story_player.next_story_in")} {autoAdvanceIn}s…
          </p>
          <div className="flex gap-3">
            <Button variant="ghost" className="border border-white/30 text-white hover:bg-white/10" onClick={onReplay}>
              <RotateCcw className="mr-2 h-3.5 w-3.5" />
              {t("components.story_player.watch_again")}
            </Button>
            <Button className="bg-white text-black hover:bg-white/90" onClick={onNext}>
              <Play className="mr-2 h-3.5 w-3.5 fill-current" />
              {t("components.story_player.play_now")}
            </Button>
          </div>
        </div>}

      {/* Loop-complete banner */}
      {showLoopBanner && <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-black/90">
          <span className="text-5xl" role="img" aria-label={t("components.story_player.party")}>🎉</span>
          <p className="text-2xl font-bold text-white">{t("components.story_player.all_stories_watched")}</p>
          <p className="text-sm text-white/60">
            {t("components.story_player.starting_over")}
          </p>
        </div>}
