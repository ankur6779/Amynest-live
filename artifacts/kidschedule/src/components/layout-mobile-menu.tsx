...
import { useDebouncedAction } from "@/lib/debounce-action";
...
function MobileMenuSheetInner({
  ...
  }, [isOpen, location, userId, isLoaded, user]);
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetTrigger asChild>
        <Button
          ...
          onClick={() => logNavEvent("menu-click", { location })}
        >
  ...
  const handleOpenChange = useCallback(
    (open: boolean) => {
      runDebounced(() => {
        try {
          if (open) {
            logNavEvent("menu-open-request", { location, userId, isLoaded });
            if (!isLoaded) {
              console.warn("[amynest:nav] User not ready — deferring menu open");
              return;
            }
          }
          setIsSidebarOpen(open);
        } catch (err) {
          logNavError("menu-open", err, { open, location });
        }
      });
    },
    [runDebounced, isLoaded, userId, location],
  );

  return (
    <AppErrorBoundary
      label="MobileMenu"
      fallback={<MenuFallbackUi />}
    >
      <Suspense fallback={<RouteLoadingShell />}>
        <MobileMenuSheetInner isOpen={isSidebarOpen} onOpenChange={handleOpenChange} />
      </Suspense>
    </AppErrorBoundary>
  );
}
