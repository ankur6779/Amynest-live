...
export function ColoringBooks({
  ...
  const fetchPage = useCallback(async (targetPage: number, signal?: AbortSignal) => {
    ...
    setListError(null);
    try {
      const res = await authFetch(`/api/coloring/list?childId=${numericChildId}&page=${targetPage}`, {
        signal
      });
      if (!res.ok) {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 429) {
          ...
          setRowError({
            id: file.id,
            message: "Please sign in again to download."
          });
        } else {
          setRowError({
            id: file.id,
  ...
  if (loading && files.length === 0 && !listError) {
    return <div className="flex items-center justify-center py-10 text-muted-foreground">
        ...
        {t("components.coloring_books.loading_coloring_books")}
      </div>;
  }
  if (listError) {
    return <Card className="bg-red-50/60 dark:bg-red-500/10 border-red-200 dark:border-red-500/30 rounded-2xl">
        <CardContent className="p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <Button variant="outline" size="sm" className="mt-2 gap-1.5" onClick={() => void fetchPage(page)} data-testid="coloring-retry-button">
              <RefreshCw className="h-3.5 w-3.5" />
              {t("components.coloring_books.try_again")}
            </Button>
          </div>
        </CardContent>
      </Card>;
  ...
  return <div className="space-y-4" data-testid="coloring-books-section">
      {/* Daily quota banner */}
      {quota && <div data-testid="coloring-quota-banner" className={["flex items-center justify-between rounded-2xl px-4 py-2.5 text-sm", quotaExhausted ? "bg-amber-50/70 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-amber-900 dark:text-amber-200" : "bg-emerald-50/70 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-900 dark:text-emerald-200"].join(" ")}>
      ...
      {allDone ? <Card className="bg-white/60 dark:bg-white/[0.04] border border-white/50 dark:border-white/10 rounded-2xl">
          <CardContent className="p-8 text-center">
            <CheckCircle2 className="h-10 w-10 text-emerald-500 mx-auto mb-3" />
            <p className="font-quicksand font-bold text-base text-foreground">
              {t("components.coloring_books.all_caught_up")}
            </p>
            <p className="text-sm text-muted-foreground mt-1">
              {childName} {t("components.coloring_books.has_downloaded_every_coloring_book_in_the_library_we_add_new")}
            </p>
          </CardContent>
        </Card> : <>
          ...
          return <Card key={file.id} data-testid={`coloring-card-${file.id}`} className="group relative overflow-hidden rounded-2xl bg-white/60 dark:bg-white/[0.04] backdrop-blur-xl border border-white/50 dark:border-white/10 shadow-[0_4px_24px_-8px_rgba(15,23,42,0.08)] hover:border-rose-300/60 hover:shadow-[0_0_0_1px_rgba(244,63,94,0.25),0_10px_36px_-10px_rgba(244,63,94,0.30)] transition-all">
                  <CardContent className="p-3">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <Button size="sm" disabled={isDownloading || quotaExhausted} onClick={() => void handleDownload(file)} data-testid={`coloring-download-${file.id}`} className="flex-1 h-8 px-2 text-[11px] gap-1 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white disabled:opacity-60">
                        {isDownloading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                        {isDownloading ? "Saving" : "Download"}
                      </Button>
                    </div>
                    {showRowError && <p className="text-[11px] text-red-600 dark:text-red-400 mt-2 flex items-start gap-1">
          ...
          {/* Pagination controls */}
          {pagination && pagination.totalPages > 1 && <div className="flex items-center justify-between pt-1">
              <Button variant="outline" size="sm" onClick={handlePrev} disabled={!pagination.hasPrev || loading} data-testid="coloring-prev-button" className="gap-1 rounded-xl">
                <ChevronLeft className="h-4 w-4" />
                {t("components.coloring_books.prev")}
              </Button>
              <Badge variant="outline" data-testid="coloring-page-indicator" className="rounded-full px-3 py-1 font-bold text-[11px]">
                {t("components.coloring_books.page")} {pagination.page + 1} of {pagination.totalPages}
                <span className="text-muted-foreground ml-1.5">
                  · {pagination.total} {t("components.coloring_books.left")}
                </span>
              </Badge>
              <Button variant="outline" size="sm" onClick={handleNext} disabled={!pagination.hasNext || loading} data-testid="coloring-next-button" className="gap-1 rounded-xl">
                {t("components.coloring_books.next")}
      ...
      if (!open) setPreviewing(null);
        ...
        <DialogContent className="max-w-3xl w-[95vw] h-[85vh] p-0 gap-0 overflow-hidden" data-testid="coloring-preview-dialog">
...
function ThumbnailWithFallback({
  ...
  if (errored) {
    return <div className="flex flex-col items-center justify-center text-rose-400 dark:text-rose-300 p-4">
        <Palette className="h-10 w-10 mb-1.5" />
        <span className="text-[10px] font-semibold uppercase tracking-wide">
          {t("components.coloring_books.pdf")}
        </span>
      </div>;
  }
  return (
