...
// ── EASY (preschool, 3–5 years) ──────────────────────────────────────────
{
  ...
  options: ["Apple", "Banana", "Grape", "Mango"],
  ...
  options: ["Cat", "Dog", "Elephant", "Rabbit"],
  ...
  options: ["Sand and Ice", "Sun and Water", "Dark and Cold", "Wind and Fire"],
  ...
  options: ["Comb", "Spoon", "Toothbrush", "Towel"],
...
// ── MEDIUM (early school, 6–10 years) ────────────────────────────────────
{
  ...
  id: "m07",
  difficulty: "medium",
  question: "What is 7² (seven squared)?",
  options: ["14", "42", "56", "49"],
  correctAnswer: "49"
}, {
  id: "m08",
  ...
  id: "m18",
  difficulty: "medium",
  question: "A triangle has angles of 60°, 60° and ___?",
  options: ["90°", "60°", "45°", "80°"],
  correctAnswer: "60°"
},
// ── HARD (pre-teen, 10–15 years) ─────────────────────────────────────────
{
  id: "h01",
  difficulty: "hard",
  question: "A train travels at 60 km/h for 2.5 hours. How far?",
  ...
  options: ["Dickens", "Austen", "Shakespeare", "Tolstoy"],
  correctAnswer: "Shakespeare"
}, {
  id: "h05",
  difficulty: "hard",
  question: "What is the speed of light (approx)?",
  options: ["200,000 km/s", "3,00,000 km/s", "1,50,000 km/s", "5,00,000 km/s"],
  correctAnswer: "3,00,000 km/s"
}, {
  id: "h06",
  difficulty: "hard",
  ...
  id: "h12",
  difficulty: "hard",
  question: "If you fold a paper in half twice, how many layers?",
  options: ["2", "4", "6", "8"],
  correctAnswer: "4"
}, {
  id: "h13",
  ...
  options: ["0°F", "100°F", "32°F", "212°F"],
  correctAnswer: "32°F"
}, {
  id: "h16",
  difficulty: "hard",
  question: "A rectangle is 12cm × 8cm. What is its perimeter?",
  options: ["40 cm", "96 cm", "32 cm", "20 cm"],
  correctAnswer: "40 cm"
}, {
  id: "h17",
  difficulty: "hard",
  question: "A palindrome reads the same forwards and backwards. Which of these is one?",
  options: ["race", "level", "tiger", "panel"],
  correctAnswer: "level"
}, {
  id: "h18",
  difficulty: "hard",
...
function FloatingStars({
  ...
  return <div style={{
    ...
    overflow: "visible"
      ...
      {stars.map((s, i) => <span key={`${trigger}-${i}`} style={{
      position: "absolute",
      left: `${15 + i * 14}%`,
      bottom: "30%",
      fontSize: 20 + i % 3 * 8,
...
function ProgressDots({
  ...
  return <div className="flex items-center justify-center gap-2">
      ...
      const res = results[i];
      return <div key={i} className="flex flex-col items-center gap-0.5">
            <span style={{
          fontSize: 14,
          opacity: res === null ? i === current ? 1 : 0.3 : 1
        }}>
...
function OptionBtn({
  ...
  return <button key={animKey} onClick={onClick} disabled={submitted} className="w-full flex items-center justify-between gap-3 px-4 py-3.5 rounded-2xl font-bold text-sm transition-all active:scale-95 text-left disabled:cursor-default" style={{
    background: bg,
...
function SessionDone({
  ...
  const msg = pct === 100 ? `🌟 Perfect score, ${childName}!` : pct >= 80 ? `🎉 Amazing job, ${childName}!` : pct >= 60 ? `👍 Great effort, ${childName}!` : `💪 Keep practising, ${childName}!`;
  return <div className="flex flex-col items-center text-center py-6 px-4" style={{
    animation: "pz-appear 400ms ease both"
  }}>
      <div style={{
      fontSize: 72,
      animation: "pz-pop 600ms cubic-bezier(0.34,1.56,0.64,1) both"
    }}>
        🏆
      </div>

      <h3 className="text-xl font-black text-white mt-3 mb-1">{t("components.daily_puzzle.session_complete")}</h3>
      ...
      <div className="flex gap-2 mb-3">
        ...
        fontSize: 28,
        animation: `pz-pop 500ms ${i * 80}ms cubic-bezier(0.34,1.56,0.64,1) both`
      }}>
            {results[i] ? "⭐" : "💔"}
          </span>)}
      </div>
      ...
      <div className="text-xs text-white/40 mb-5">
        {t("components.daily_puzzle.difficulty")} {DIFF_CFG[difficulty].stars} {DIFF_CFG[difficulty].label}
      </div>
      ...
      <p className="text-[11px] text-white/30 mt-4">{t("components.daily_puzzle.new_puzzles_unlock_tomorrow_come_back_for_more")}</p>
    </div>;
}
  ...
  const handleSubmit = useCallback(() => {
    ...
    setState(prev => {
      ...
      if (newDiff !== prev.difficulty) {
        setLevelMsg(newDiff === "medium" ? "🎉 Level Up! Medium unlocked!" : newDiff === "hard" ? "🚀 Hard difficulty unlocked!" : "👍 Dropping to easier — you've got this!");
      }
      const next: PuzzleState = {
        ...prev,
        difficulty: newDiff,
  ...
  return <div className="rounded-3xl overflow-hidden" style={{
    background: "linear-gradient(160deg,hsl(var(--brand-indigo-950)) 0%,#0f0f1a 100%)"
      ...
      <div className="px-4 pt-4 pb-2 flex items-center justify-between">
        <div className="text-[11px] font-black px-2.5 py-1 rounded-full" style={{
        background: diff.bg,
        color: diff.color
      }}>
      ...
      {state.difficulty === "hard" && !submitted && <div className="px-4 pb-2">
          <TimerBar seconds={30} onExpire={handleTimerExpire} running={timerRunning} />
        </div>}

      {/* ── Question area ─────────────────────────────────────────────── */}
      <div key={questionKey} className="px-4 pt-2 pb-4 relative" style={{
      animation: "pz-appear 300ms ease both"
    }}>
        {/* Visual emoji hint (easy puzzles) */}
        {cur.visual && !submitted && <div className="flex items-center justify-center mb-3">
            <div className="px-5 py-3 rounded-2xl border text-3xl tracking-widest" style={{
        ...
        {/* Question text */}
        <p className="text-white font-black text-lg leading-snug text-center mb-4" style={{
        ...
        <div className="grid grid-cols-1 gap-2.5">
          {cur.options.map(opt => <OptionBtn key={opt} label={opt} selected={selected === opt} submitted={submitted} isCorrect={opt === cur.correctAnswer} isWrongSelected={isWrong && selected === opt} onClick={() => {
      ...
      {/* ── Stats footer ──────────────────────────────────────────────── */}
      <div className="px-4 pb-3 flex items-center justify-between text-[11px]" style={{
      color: "rgba(255,255,255,0.35)"
    }}>
        <span>{t("components.daily_puzzle.all_time")} <strong className="text-white/60">{state.totalCorrect}</strong> {t("components.daily_puzzle.solved")}</span>
        <span>{t("components.daily_puzzle.accuracy")} <strong className="text-white/60">{state.totalAttempted > 0 ? Math.round(state.totalCorrect / state.totalAttempted * 100) : 0}%</strong></span>
      </div>
    </div>;
}
