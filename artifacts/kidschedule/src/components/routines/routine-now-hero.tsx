 ...
 * Living ON: presence meter (no %) + companion language.
 * Living OFF: legacy % ring retained for rollback.
 */
    ...
    >
      ...
      <div className="relative flex items-center gap-4">
        <div
          ...
          aria-label={
            living
              ? t("routines.living.execution.presence_aria", {
                  defaultValue: "Gentle presence through the day",
                })
              : t("pages.routines.detail.progress_aria", {
                  defaultValue: "{{pct}} percent complete",
                  pct,
                })
          }
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            {living ? (
              <>
                <span className="font-quicksand text-sm font-bold leading-none text-foreground">
                  {livingPresenceRingLabel(completed, total)}
                </span>
                <span className="text-[9px] font-semibold uppercase tracking-wide text-foreground/55 mt-0.5">
                  {livingPresenceRingHint(completed, total)}
                </span>
              </>
            ) : (
              <>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.16em] text-amber-300/90">
            ...
            <span className="truncate">
              {living
                ? childName
                  ? t("routines.living.execution.hero_beside", {
                      name: childName,
                      defaultValue: livingHeroChildDay(childName),
                    })
                  : t("routines.living.execution.hero_today", {
                      defaultValue: "Beside you today",
                    })
                : childName
                  ...
                  : t("pages.routines.detail.hero_todays_day", {
          ...
          {focus.kind === "done" ? (
            <p className="mt-1.5 font-quicksand text-lg font-bold text-foreground leading-tight">
              {living
                ? t("routines.living.execution.hero_done", {
                    defaultValue: livingHeroDoneTitle(),
                  })
            ...
            <>
              <div className="mt-1.5 flex items-center gap-1.5">
                {focus.kind === "now" && (
                  <span
                    className={cn(
                      "inline-flex h-2 w-2 shrink-0 rounded-full bg-amber-400",
                ...
                <span className="text-[11px] font-bold uppercase tracking-wide text-foreground/60">
                  {focus.kind === "now"
                    ? living
                      ? t("routines.living.execution.hero_now", {
                          defaultValue: livingHeroNowLabel(),
                        })
                      : t("pages.routines.detail.hero_happening_now", {
                          defaultValue: "Happening now",
                        })
                    : living
                      ? t("routines.living.execution.hero_next", {
                          defaultValue: livingHeroNextLabel(),
                        })
                      : t("pages.routines.detail.hero_up_next", {
