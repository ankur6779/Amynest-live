...
import { QUICK_BEHAVIORS, QUICK_BEHAVIOR_KEYS, TRIGGERS, TRIGGER_KEYS, SOLUTIONS, SITUATION_HELP, UI_LABELS, buildAmyInsights, computeScore, scoreLabel, encodeTriggerNote, type LangKey, type QuickBehaviorKey, type TriggerKey } from "@workspace/behavior-tracker";
...
function Block({
  ...
  return <div className={["rounded-2xl overflow-hidden transition-all duration-300", "bg-white/60 dark:bg-white/[0.04] backdrop-blur-xl", "border border-white/50 dark:border-white/10", "shadow-[0_4px_24px_-8px_rgba(15,23,42,0.08)]", !alwaysOpen && "hover:border-primary/40 dark:hover:border-primary/40", !alwaysOpen && "hover:shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_10px_36px_-10px_rgba(168,85,247,0.35)]", open && "border-primary/60 dark:border-primary/50 shadow-[0_0_0_1px_rgba(168,85,247,0.45),0_18px_50px_-12px_rgba(168,85,247,0.45)]"].join(" ")}>
      <button onClick={onToggle} disabled={alwaysOpen} className={["w-full flex items-center justify-between gap-3 px-4 py-3.5 text-left transition-colors duration-200", open ? "bg-primary/[0.06] dark:bg-primary/[0.08]" : "hover:bg-white/40 dark:hover:bg-white/[0.03]", alwaysOpen ? "cursor-default" : "cursor-pointer"].join(" ")}>
        ...
        {!alwaysOpen && <span className={["shrink-0 w-7 h-7 rounded-full flex items-center justify-center border bg-white/50 dark:bg-white/5 transition-transform duration-300", open ? "rotate-180 text-primary border-primary/40" : "text-muted-foreground border-border/50"].join(" ")}>
            <ChevronDown className="h-4 w-4" />
          </span>}
      </button>
      {(alwaysOpen || open) && <div className="px-4 pb-5 pt-3 border-t border-white/40 dark:border-white/10 bg-white/30 dark:bg-white/[0.015] animate-in fade-in slide-in-from-top-1 duration-300">
          {children}
        </div>}
    </div>;
}
...
export default function BehaviorTracker() {
  ...
  } = useListBehaviors(todayParams, {
    query: {
      queryKey: getListBehaviorsQueryKey(todayParams)
    }
  });
  const {
    data: allLogs = []
  } = useListBehaviors(undefined, {
    query: {
      queryKey: getListBehaviorsQueryKey()
    }
  });
  ...
  function quickLog(key: QuickBehaviorKey) {
    ...
    createMutation.mutate({
      ...
      onSuccess: () => {
        toast({
          title: `${def.emoji} ${def.label[lang]} logged!`
        });
      ...
      onError: (err: unknown) => {
        ...
        toast({
          title: t("toasts.behavior.log_failed"),
          variant: "destructive"
        });
      }
    });
  ...
  function handleDelete(id: number) {
    deleteMutation.mutate({
      ...
      onError: (err: unknown) => {
        ...
        toast({
          title: t("toasts.behavior.delete_failed"),
          variant: "destructive"
        });
      }
    });
  ...
  const insightLogs = useMemo(() => effectiveChildId ? allLogs.filter(l => l.childId === effectiveChildId) : allLogs, [allLogs, effectiveChildId]);
  ...
  return <div className="flex flex-col gap-5 animate-in fade-in duration-500 pb-12">
      {/* Header */}
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="font-quicksand text-2xl font-bold text-foreground">{t("pages.behavior.index.behavior_tracker")}</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setSituationKey("crying")} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300 text-xs font-bold hover:bg-rose-200 dark:hover:bg-rose-500/30 transition-colors border border-rose-200 dark:border-rose-500/30">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        const chip = <button onClick={() => {
          if (locked) return;
          setSelectedChild(v => v === c.id ? null : c.id);
        }} disabled={locked} className={["px-4 py-2 rounded-xl text-sm font-bold border-2 transition-all", selectedChild === c.id ? "bg-primary text-primary-foreground border-primary shadow-[0_0_0_2px_rgba(168,85,247,0.3)]" : "bg-white/60 dark:bg-white/5 border-border/60 hover:border-primary/40"].join(" ")}>
              {c.name}
            </button>;
        return <LockedBlock key={c.id} locked={locked} reason="child_limit" rounded="rounded-xl">
              {chip}
            </LockedBlock>;
      })}
        {children.length === 0 && <p className="text-sm text-muted-foreground py-2">{t("pages.behavior.index.add_a_child_profile_to_start_tracking")}</p>}
      </div>
      ...
      {children.length > 1 && <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        ...
        const card = <div className="rounded-2xl p-3 bg-white/60 dark:bg-white/5 border border-border/50">
        ...
        const locked = !isPremium && index > 0;
        return <LockedBlock key={c.id} locked={locked} reason="child_limit" rounded="rounded-2xl">
                {card}
              </LockedBlock>;
      })}
        </div>}
      ...
      <div className="flex flex-col gap-3">
        {/* 1. Quick Log */}
        <Block icon={<Zap className="h-5 w-5 text-amber-600" />} title={L.quickLog} subtitle={L.tap1Log} iconBg="bg-amber-100 dark:bg-amber-500/20" open={openBlock === "quick-log"} onToggle={() => toggle("quick-log")}>
          <div className="space-y-4">
            <div className="grid grid-cols-4 gap-2 sm:grid-cols-7">
              {QUICK_BEHAVIOR_KEYS.map(key => {
              const def = QUICK_BEHAVIORS[key];
              return <button key={key} onClick={() => quickLog(key)} disabled={createMutation.isPending} className={["flex flex-col items-center gap-1.5 p-2.5 rounded-xl border transition-all", "bg-white/60 dark:bg-white/5 border-border/50", "hover:scale-105 hover:shadow-md active:scale-95"].join(" ")} style={{
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {TRIGGER_KEYS.map(k => {
                const t = TRIGGERS[k];
                return <button key={k} onClick={() => setPendingTrigger(v => v === k ? null : k)} className={["flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all", pendingTrigger === k ? "bg-primary/10 border-primary text-primary" : "bg-white/50 dark:bg-white/5 border-border/50 text-muted-foreground hover:border-primary/40"].join(" ")}>
            ...
            {/* Today log list */}
            {todayLoading ? <div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div> : todayLogs.length > 0 ? <div className="space-y-2 border-t border-border/30 pt-3">
              ...
              return <div key={log.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-white/40 dark:bg-white/5 border border-border/30 group">
                      ...
                      <button onClick={() => handleDelete(log.id)} disabled={deleteMutation.isPending} className="opacity-0 group-hover:opacity-100 p-1 text-muted-foreground hover:text-destructive transition-all">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>;
            })}
              </div> : <p className="text-xs text-muted-foreground text-center py-2">{L.noDataYet}</p>}
          </div>
        </Block>

        {/* 2. Today Summary */}
        <Block icon={<span className="text-xl">📊</span>} title={L.todaySummary} subtitle={`${todayLogs.length} ${L.loggedToday}`} iconBg="bg-blue-100 dark:bg-blue-500/20" open={openBlock === "summary"} onToggle={() => toggle("summary")}>
          <div className="space-y-3">
            ...
            <div className="rounded-xl bg-gradient-to-r from-primary/10 to-violet-500/10 border border-primary/20 p-4 text-center">
              ...
              <div className="mt-2 h-2 rounded-full bg-white/50 dark:bg-white/10 overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-primary to-violet-500 transition-all duration-700" style={{
                width: `${score}%`
              }} />
              </div>
            </div>
        ...
        {/* 3. Amy Insights */}
        <Block icon={<Brain className="h-5 w-5 text-violet-600" />} title={L.amyInsights} subtitle={insights.length > 0 ? `${insights.length} pattern${insights.length > 1 ? "s" : ""} found` : "Log more to unlock"} iconBg="bg-violet-100 dark:bg-violet-500/20" open={openBlock === "insights"} onToggle={() => toggle("insights")}>
          {insights.length === 0 ? <p className="text-sm text-muted-foreground py-2 text-center">{L.noInsights}</p> : <div className="space-y-3">
              {insights.map((ins, i) => <div key={i} className="flex gap-3 p-3 rounded-xl bg-violet-50 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-500/20">
        ...
        {/* 4. Weekly Trends */}
        <Block icon={<BarChart2 className="h-5 w-5 text-blue-600" />} title={L.weeklyTrends} subtitle="Last 7 days" iconBg="bg-blue-100 dark:bg-blue-500/20" open={openBlock === "trends"} onToggle={() => toggle("trends")}>
          <div className="flex items-end gap-1.5 h-28">
            {weekData.map((d, i) => <div key={i} className="flex-1 flex flex-col items-center gap-1">
              ...
              height: "80px"
                  ...
                  {d.pos > 0 && <div className="w-full rounded-t-sm bg-emerald-400 dark:bg-emerald-500" style={{
                height: `${d.pos / maxWeek * 80}px`,
                minHeight: 4
              }} />}
                  {d.neg > 0 && <div className="w-full rounded-b-sm bg-red-400 dark:bg-red-500" style={{
                height: `${d.neg / maxWeek * 80}px`,
                minHeight: 4
              }} />}
                  {d.total === 0 && <div className="w-full rounded bg-border/40" style={{
                height: 4
              }} />}
                </div>
        ...
        {/* 5. Solutions & Tips */}
        <Block icon={<Lightbulb className="h-5 w-5 text-orange-500" />} title={L.solutions} subtitle="Amy's proven tips per situation" iconBg="bg-orange-100 dark:bg-orange-500/20" open={openBlock === "solutions"} onToggle={() => toggle("solutions")}>
          <div className="space-y-4">
            ...
            return <div key={key}>
                  ...
                  <div className="space-y-1.5">
                    {tips.map((tip, i) => <div key={i} className="flex gap-2.5 items-start">
                        <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 text-[10px] font-black text-white" style={{
      ...
      {situationKey && <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="relative z-10 w-full max-w-md rounded-3xl bg-white dark:bg-[#14142B] border border-border/50 p-6 shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
            ...
            const labels: Record<string, string> = {
              crying: L.childHelp,
              angry: L.childAngry,
              not_listening: L.childNotListening
            };
            return <button key={k} onClick={() => setSituationKey(k)} className={["flex-1 py-2 rounded-xl text-xs font-bold border transition-all", situationKey === k ? "bg-primary text-primary-foreground border-primary" : "bg-muted text-muted-foreground border-border hover:border-primary/50"].join(" ")}>
                    {labels[k]}
                  </button>;
          })}
            </div>
            <div className="space-y-3">
              {SITUATION_HELP[situationKey][lang].map((tip, i) => <div key={i} className="flex gap-3 items-start p-3 rounded-xl bg-primary/5 border border-primary/15">
