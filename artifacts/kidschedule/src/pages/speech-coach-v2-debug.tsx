/**
 * Standalone OpenAI Realtime debug page — NOT Speech Coach V2 integration.
 * Route: /speech-coach-v2-debug (dev only)
 */
...
export default function SpeechCoachV2DebugPage() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!mic.ok) {
        pushTrace("MIC_REQUEST_FAILURE", { reason: mic.reason });
        throw new Error(`Microphone unavailable: ${mic.reason}`);
      }
      pushTrace("MIC_REQUEST_SUCCESS", { tracks: mic.stream.getAudioTracks().length });
      streamRef.current = mic.stream;

      const tokenRes = await fetch(getApiUrl("/api/speech/v2/debug/realtime-token"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          instructions: "You are Amy. Say hello warmly in one sentence when the session starts.",
        }),
      });
      ...
      dc.onopen = () => {
        ...
        pushTrace("RESPONSE_CREATE_SENT");
        refreshTrace();
      };
      ...
      dc.onmessage = (event) => {
        try {
          ...
          if (type === "session.created") pushTrace("session.created", undefined, payload);
          if (type === "session.updated") pushTrace("session.updated", undefined, payload);
          if (type === "response.created") pushTrace("response.created", undefined, payload);
          if (type === "response.done") pushTrace("response.done", undefined, payload);
          if (type === "error") pushTrace("error", undefined, payload);
          refreshTrace();
        } catch (err) {
      ...
      pc.ontrack = (event) => {
        ...
        void audioEl.play().then(() => {
          ...
          pushTrace("AUDIO_PLAY_STARTED", state);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6 font-mono text-sm">
      <h1 className="text-xl font-bold font-sans">Speech Coach V2 — OpenAI Realtime Debug</h1>
      <p className="text-muted-foreground font-sans">
        Direct mic → OpenAI Realtime → Amy voice. No lesson engine, quota, or analytics.
      </p>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground font-sans">
          Also on <code>window.__OPENAI_REALTIME_TRACE__</code>
        </p>
