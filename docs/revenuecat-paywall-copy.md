# RevenueCat Paywall — v2 Production Copy

## Headline

The growth system for ages 0–12

## Subheadline

Amy AI · Coach · Speech Coach · Learning · Routines · Hub—one subscription. Not scattered apps.

## Default package

`six_month` — Steady Progress · Safest Choice

## Annual nudge (template text)

Most savings—tap Growth Year

---

## Try the System (Monthly)

**Subtitle:** Full ecosystem. One month. Zero lock-in.  
**Value:** Less than a single tutoring session—entire platform included.  
**CTA:** Try one month

- Full AmyNest ecosystem for 30 days—nothing held back
- See calmer routines and clearer parenting within the first two weeks
- Try Speech Coach and learning paths with your child this month
- Cancel in two taps—only continue if home feels different

---

## Steady Progress (6 Months) · Safest Choice

**Subtitle:** Safest choice for real habit change.  
**Value:** Save ~17% vs monthly—most families land here.  
**CTA:** Start steady progress

- Enough time for routines to stick and communication to open up
- Track learning, behavior, and speech progress across months—not days
- Coach + Hub evolve with your child's stage
- Meaningful savings vs paying monthly—without a full-year decision yet

---

## Growth Year (Annual) · Smartest Choice

**Subtitle:** Smartest choice—best savings, zero gaps.  
**Value:** Save ~33% vs monthly · ~$20 less than six months twice.  
**CTA:** Choose Growth Year

- Lowest cost per month—protect the budget while maximizing support
- Unbroken coaching, speech practice, and learning through the full year
- Routines and Hub content that mature with your child ages 0–12
- Never re-decide mid-year when life gets busy—growth stays on

---

## Primary CTA

Unlock the full system

## Restore

Restore Purchases

## Footer

Auto-renews via Google Play until cancelled in Subscriptions.

## Feature bullets

- In-the-moment guidance—not articles you'll never read
- Step-by-step plans for tantrums, screens, sleep, focus
- Pronunciation practice that builds real speaking confidence
- Activities at their level—harder as they grow
- Life skills & stories—not random YouTube rabbit holes
- Mornings & bedtimes that reduce nagging and stress
- Patterns that help you respond—not react
- Proof of growth for you and pride for them
