...
/**
 * Premium PDF export history — store generated bytes so reopen never regenerates
 * unless the parent explicitly requests a new export.
 */
export const birthSkyPdfExportsTable = pgTable(
  "birth_sky_pdf_exports",
  {
    id: text("id").primaryKey(),
    profileId: text("profile_id").notNull(),
    userId: text("user_id").notNull(),
    ...
    /** base64-encoded PDF bytes (stored once; reopen serves this). */
    pdfBase64: text("pdf_base64").notNull(),
    byteSize: integer("byte_size").notNull(),
    chartDetailsVersion: text("chart_details_version"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => ({
    profileIdx: index("birth_sky_pdf_exports_profile_idx").on(t.profileId),
    userIdx: index("birth_sky_pdf_exports_user_idx").on(t.userId),
    snapshotIdx: index("birth_sky_pdf_exports_snapshot_idx").on(t.snapshotId),
  }),
);
...
export type BirthSkyPdfExportRow = typeof birthSkyPdfExportsTable.$inferSelect;
