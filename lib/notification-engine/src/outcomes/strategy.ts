...
export interface OutcomeNotificationDraft {
  ...
  priority: number;
  source: "streak_recovery" | "campaign" | "milestone" | "learning" | "routine" | "conversion" | "retention" | "category_default";
}
...
export function resolveOutcomeStrategy(input: ResolveStrategyInput): OutcomeNotificationDraft | null {
  ...
  // Priority 3: Churn / retention intervention
  if (
    signals.churnRisk7d >= 0.6 &&
    stageStrategy.interventionIntensity !== "low"
  ) {
    const milestoneCopy = milestoneJourneyCopy("AT_RISK", signals.childName);
    if (milestoneCopy) {
      ...
      return {
        ...
        goal: "GOAL_RETENTION",
        dedupKey: `retention:${input.localDate}`,
        recommendationKey: "retention:at_risk",
        outcome: { ...baseOutcome, goal: "GOAL_RETENTION" },
        priority: 85,
        source: "retention",
      };
    }
