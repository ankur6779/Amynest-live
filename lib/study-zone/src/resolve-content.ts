...
/** Play mode age bands — younger kids see fewer items per category. */
export function getPlayCategoriesForChild(
  ...
  // Level 1 (ages 2–3): simplified sets. Level 2+ (4–5): full categories.
  if (band >= 2) return cats;

  return cats.map((cat) => {
    switch (cat.id) {
      case "alphabets":
        return { ...cat, items: cat.items.filter((i) => ["A", "B", "C", "D", "E"].includes(i.id)) };
      case "numbers":
        return { ...cat, items: cat.items.filter((i) => parseInt(i.id, 10) <= 5) };
      case "colors":
        return { ...cat, items: cat.items.slice(0, 6) };
      case "shapes":
        return { ...cat, items: cat.items.filter((i) => ["circle", "square", "triangle"].includes(i.id)) };
      case "animals":
        return { ...cat, items: cat.items.slice(0, 6) };
      case "fruits":
        return { ...cat, items: cat.items.slice(0, 6) };
      case "rhymes":
        return { ...cat, items: cat.items.slice(0, 3) };
      default:
        return cat;
    }
  });
