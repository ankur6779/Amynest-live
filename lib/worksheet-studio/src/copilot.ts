...
const RULES: Array<{ re: RegExp; result: CopilotResult }> = [
  ...
  { re: /optimiz\w*\s+(for\s+)?print|print optim/i, result: { kind: "message", text: "Ask: Optimize for printing" } },
  { re: /replace emojis?|black[- ]?outline|printable.*(illustration|image)|line art/i, result: { kind: "action", action: "replace_images" } },
  { re: /printable/i, result: { kind: "message", text: "Use Export → PDF, or say “Optimize for printing”." } },
